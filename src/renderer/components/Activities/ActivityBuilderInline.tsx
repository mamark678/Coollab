    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', padding: '32px 48px', background: 'transparent' }}>
      ...
      <div style={{ marginBottom: 28, textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 6 }}>
          <Sparkles size={22} color="var(--theme-primary)" />
          <span style={{ fontSize: 22, fontWeight: 700, color: 'var(--theme-text-primary)' }}>Activity Builder</span>
        </div>
        ...
        {existingCount > 0 && (
          <div style={{ fontSize: 12, color: 'var(--theme-primary)', marginTop: 6 }}>
      ...
      {/* Mode Toggle */}
      <div style={{ maxWidth: 600, width: '100%', margin: '0 auto 24px', display: 'flex', background: 'color-mix(in srgb, var(--theme-text-primary) 5%, transparent)', borderRadius: 10, padding: 4, border: '1px solid var(--theme-border)' }}>
        <button onClick={() => setMode('ai')} style={{
          ...
          background: mode === 'ai' ? 'color-mix(in srgb, var(--theme-primary) 15%, transparent)' : 'transparent',
          color: mode === 'ai' ? 'var(--theme-primary)' : 'var(--theme-text-secondary)',
          boxShadow: mode === 'ai' ? '0 2px 8px color-mix(in srgb, var(--theme-primary) 20%, transparent)' : 'none',
        }}>
          <Sparkles size={15} /> AI Driven
        </button>
        <button onClick={() => setMode('manual')} style={{
          flex: 1, padding: '10px 16px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s',
          background: mode === 'manual' ? 'color-mix(in srgb, var(--theme-primary) 15%, transparent)' : 'transparent',
          color: mode === 'manual' ? 'var(--theme-primary)' : 'var(--theme-text-secondary)',
          boxShadow: mode === 'manual' ? '0 2px 8px color-mix(in srgb, var(--theme-primary) 20%, transparent)' : 'none',
        }}>
      ...
      <div style={{ maxWidth: 600, width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        {mode === 'manual' ? (
          ...
          <>
            {/* Topic */}
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--theme-text-primary)', marginBottom: 6, display: 'block' }}>
              ...
              <textarea value={topic} onChange={e => setTopic(e.target.value)}
                ...
                style={{ width: '100%', minHeight: 80, padding: '10px 14px', background: 'color-mix(in srgb, var(--theme-text-primary) 3%, transparent)', border: '1px solid var(--theme-border)', borderRadius: 8, color: 'var(--theme-text-primary)', fontSize: 13, lineHeight: 1.6, resize: 'vertical', outline: 'none', transition: 'border-color 0.2s' }}
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="number" min={1} max={10} value={numActivities}
                  ...
                  style={{ width: '100%', padding: '8px 12px', background: 'color-mix(in srgb, var(--theme-text-primary) 3%, transparent)', border: '1px solid var(--theme-border)', borderRadius: 8, color: 'var(--theme-text-primary)', fontSize: 14, outline: 'none' }}
                />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--theme-text-primary)', marginBottom: 6, display: 'block' }}>
                ...
                <input type="number" min={1} max={60} step={1} value={durationMinutes}
                  ...
                  style={{ width: '100%', padding: '8px 12px', background: 'color-mix(in srgb, var(--theme-text-primary) 3%, transparent)', border: '1px solid var(--theme-border)', borderRadius: 8, color: 'var(--theme-text-primary)', fontSize: 14, outline: 'none' }}
                />
                <div style={{ fontSize: 11, color: 'var(--theme-text-secondary)', marginTop: 3 }}>{durationMinutes} minute{durationMinutes !== 1 ? 's' : ''} per activity</div>
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--theme-text-primary)', marginBottom: 6, display: 'block' }}>
                ...
                <select value={difficulty} onChange={e => setDifficulty(e.target.value as any)}
                  style={{ width: '100%', padding: '8px 12px', background: 'color-mix(in srgb, var(--theme-text-primary) 3%, transparent)', border: '1px solid var(--theme-border)', borderRadius: 8, color: 'var(--theme-text-primary)', fontSize: 14, outline: 'none', cursor: 'pointer' }}>
                  <option value="easy" style={{ background: 'var(--theme-surface)', color: 'var(--theme-text-primary)' }}>Beginner</option>
                  <option value="medium" style={{ background: 'var(--theme-surface)', color: 'var(--theme-text-primary)' }}>Medium</option>
                  <option value="hard" style={{ background: 'var(--theme-surface)', color: 'var(--theme-text-primary)' }}>Hard</option>
                </select>
              </div>
            </div>
            ...
            {/* Activity Types */}
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--theme-text-primary)', marginBottom: 8, display: 'block' }}>Activity Types (workspace zones)</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {typeOptions.map(opt => (
                  <button key={opt.value} onClick={() => toggleActivityType(opt.value)}
                    style={{ 
                      padding: '8px 16px', 
                      background: activityTypes.includes(opt.value) ? 'color-mix(in srgb, var(--theme-primary) 15%, transparent)' : 'color-mix(in srgb, var(--theme-text-primary) 3%, transparent)', 
                      border: activityTypes.includes(opt.value) ? '1px solid var(--theme-primary)' : '1px solid var(--theme-border)', 
                      borderRadius: 8, 
                      color: activityTypes.includes(opt.value) ? 'var(--theme-primary)' : 'var(--theme-text-secondary)', 
                      fontSize: 13, 
                      fontWeight: 500, 
                      cursor: 'pointer', 
                      display: 'flex', 
            ...
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--theme-text-primary)', marginBottom: 6, display: 'block' }}>
                Additional Notes <span style={{ fontWeight: 400, color: 'var(--theme-text-secondary)' }}>(optional)</span>
              </label>
              <input type="text" value={additionalNotes} onChange={e => setAdditionalNotes(e.target.value)}
                ...
                style={{ width: '100%', padding: '8px 14px', background: 'color-mix(in srgb, var(--theme-text-primary) 3%, transparent)', border: '1px solid var(--theme-border)', borderRadius: 8, color: 'var(--theme-text-primary)', fontSize: 13, outline: 'none' }}
              />
            </div>

            {/* Status / Error / Success */}
            {statusMessage && (
              <div style={{ padding: '10px 14px', background: 'color-mix(in srgb, var(--theme-primary) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--theme-primary) 20%, transparent)', borderRadius: 8, color: 'var(--theme-primary)', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> {statusMessage}
              </div>
            )}
            {error && (
              <div style={{ padding: '10px 14px', background: 'color-mix(in srgb, var(--theme-error, #ef4444) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--theme-error, #ef4444) 20%, transparent)', borderRadius: 8, color: 'var(--theme-error, #ef4444)', fontSize: 13 }}>{error}</div>
            )}
            {successMessage && (
              <div style={{ padding: '10px 14px', background: 'color-mix(in srgb, var(--theme-success, #10b981) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--theme-success, #10b981) 20%, transparent)', borderRadius: 8, color: 'var(--theme-success, #10b981)', fontSize: 13 }}>✓ {successMessage}</div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleGenerate} disabled={isGenerating} style={{
                ...
                background: isGenerating ? 'color-mix(in srgb, var(--theme-primary) 40%, transparent)' : 'linear-gradient(135deg, var(--theme-primary), color-mix(in srgb, var(--theme-primary) 85%, black))',
                border: 'none', borderRadius: 10, color: 'var(--theme-on-primary)', fontSize: 15, fontWeight: 600,
                cursor: isGenerating ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                transition: 'all 0.2s', boxShadow: isGenerating ? 'none' : '0 4px 20px color-mix(in srgb, var(--theme-primary) 30%, transparent)'
              }}>
                {isGenerating ? (
              ...
              <button onClick={resetAIForm} style={{
                ...
                color: 'var(--theme-text-secondary)', fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s',
              }}>Cancel</button>
            </div>
          </>
        )}
        ...
        <div ref={resultRef} style={{ maxWidth: 600, width: '100%', margin: '32px auto 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            {generatedActivities.some(g => g.status === 'preview') && (
              <button onClick={() => handleSaveAll()} style={{ padding: '6px 16px', background: 'color-mix(in srgb, var(--theme-success, #10b981) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--theme-success, #10b981) 30%, transparent)', borderRadius: 6, color: 'var(--theme-success, #10b981)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                <Check size={12} style={{ marginRight: 4, verticalAlign: -1 }} /> Save All to Project
              </button>
            )}
          </div>
          {generatedActivities.map((gen, idx) => (
            <div key={gen.id} style={{ background: gen.status === 'saved' ? 'color-mix(in srgb, var(--theme-success, #10b981) 4%, transparent)' : 'color-mix(in srgb, var(--theme-text-primary) 2%, transparent)', border: gen.status === 'saved' ? '1px solid color-mix(in srgb, var(--theme-success, #10b981) 20%, transparent)' : '1px solid var(--theme-border)', borderRadius: 10, padding: '16px 20px', marginBottom: 10, transition: 'all 0.2s' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: gen.status === 'saved' ? 'var(--theme-success, #10b981)' : 'color-mix(in srgb, var(--theme-primary) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--theme-text-primary)', flexShrink: 0 }}>
                    {gen.status === 'saved' ? <Check size={12} /> : idx + 1}
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--theme-text-primary)' }}>{gen.activity.title}</div>
                </div>
                  ...
                  {gen.status === 'preview' && (
                    <>
                      <button 
                        ...
                        style={{ padding: '4px 8px', background: 'transparent', border: '1px solid var(--theme-border)', borderRadius: 6, color: 'var(--theme-text-secondary)', fontSize: 11, cursor: 'pointer' }}
                        title="Edit Activity"
                      >
                        <PenTool size={10} />
                      </button>
                      <button onClick={() => handleSaveActivity(gen, idx)} style={{ padding: '4px 12px', background: 'color-mix(in srgb, var(--theme-success, #10b981) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--theme-success, #10b981) 30%, transparent)', borderRadius: 6, color: 'var(--theme-success, #10b981)', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                        <Check size={10} style={{ marginRight: 3 }} /> Save
                      </button>
                      <button onClick={() => handleRemoveGenerated(gen.id)} style={{ padding: '4px 8px', background: 'transparent', border: '1px solid var(--theme-border)', borderRadius: 6, color: 'var(--theme-text-secondary)', fontSize: 11, cursor: 'pointer' }}>
                        <Trash2 size={10} />
                      </button>
                    </>
                  )}
                  {gen.status === 'saving' && (<span style={{ fontSize: 11, color: 'var(--theme-primary)', display: 'flex', alignItems: 'center', gap: 4 }}><Loader2 size={12} style={{ animation: 'spin 1s linear infinite' }} /> Saving...</span>)}
                  {gen.status === 'saved' && (<span style={{ fontSize: 11, color: 'var(--theme-success, #10b981)', fontWeight: 600 }}>✓ Saved</span>)}
                </div>
              </div>
              <div style={{ fontSize: 12, color: 'var(--theme-text-secondary)', lineHeight: 1.5, marginBottom: 8 }}>{gen.activity.description}</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'color-mix(in srgb, var(--theme-primary) 10%, transparent)', color: 'var(--theme-primary)' }}>{gen.activity.workspace_zone}</span>
                <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'color-mix(in srgb, var(--theme-secondary, #3b82f6) 10%, transparent)', color: 'var(--theme-secondary, #3b82f6)' }}>{gen.activity.difficulty}</span>
                <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'color-mix(in srgb, var(--theme-success, #10b981) 10%, transparent)', color: 'var(--theme-success, #10b981)' }}>{gen.activity.points || 10} pts</span>
                {gen.activity.instructions?.length > 0 && (
                  <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'color-mix(in srgb, var(--theme-text-primary) 4%, transparent)', color: 'var(--theme-text-secondary)' }}>{gen.activity.instructions.length} instruction{gen.activity.instructions.length > 1 ? 's' : ''}</span>
                )}
              </div>
            </div>
          ))}
